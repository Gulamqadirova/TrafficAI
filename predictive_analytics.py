...
def _forecast_metric(zdf: pd.DataFrame, metric: str, horizon: int):
    ...
    yhat = np.clip(np.round(m.predict(_time_features(fmod))), 0, None).astype(int)
...
def forecast_zone(zone_df: pd.DataFrame, horizon: int = config.FORECAST_HORIZON) -> Dict:
    ...
    fut, car_hat, car_mae = _forecast_metric(z, "car_count", horizon)
    _,   ped_hat, ped_mae = _forecast_metric(z, "person_count", horizon)
